...
if test -z $TABLENAME; then
  mysql -h tomcat_test_mysql_1 -u user -puserpass programming -e "select * from ${FRONTABLENAME};" >& ${mysql_dir}/table_show.txt
else
  mysql -h tomcat_test_mysql_1 -u user -puserpass programming -e "select * from ${TABLENAME};" >& ${mysql_dir}/table_show.txt
fi
