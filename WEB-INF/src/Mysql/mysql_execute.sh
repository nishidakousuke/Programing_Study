...
if [ ${TABLECREATECHECK} -gt 0 ]; then
    TABLENUM=`mysql -h tomcat_test_mysql_1 -u root -prootpass programming -e "SELECT COUNT(*) FROM information_schema.tables WHERE table_schema = 'programming';" | sed -n 2p`
